...
c
c           f(stp) <= f(0) + ftol*stp*f'(0),  // On line 3573
c
c     and the curvature condition
c
c           abs(f'(stp)) <= gtol*abs(f'(0)).  // On line 3589
c
...
c     satisfies the sufficient decrease condition.
c
c     A typical invocation of lineww has the following outline:
c
c     task = 'START'
c  10 continue
c        call lineww( ... )
...
c     The subroutine statement is
c
c        subroutine lineww(f,g,stp,ftol,gtol,xtol,stpmin,stpmax,
c                          task,isave,dsave)
c     where
c
c       f    is a double precision variable.
...
c
c            If task(1:2) = 'FG' then evaluate the function and 
c            derivative at stp and call lineww again.
c
c            If task(1:4) = 'CONV' then the search is successful.
c
c            If task(1:4) = 'WARN' then the subroutine is not able
...
d     ouble precision :: ptemp, eta, delta, mu0, tolmu, tolrs, kmu, nQ, krs, ap, ad, Mm, r2, rs, mu, sig, dummy
d     ouble precision :: r5, r6, dy, muaff, y
d     ouble precision, dimension(n)    :: x, z, zdx, KT, r1, r3, r4, r7, e, work, dx, dz, p
d     ouble precision, dimension(m)    :: d
d     ouble precision, dimension(n, n) :: Q, QD, C, invTrC, invC
